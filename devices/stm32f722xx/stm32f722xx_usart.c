...
*/
/* Routines used to provide STM32F722xx USART functionality to the mri debugger. */
#include <stdlib.h>
#include <core/libc.h>
#include <core/platforms.h>
#include <architectures/armv7-m/debug_cm3.h>
#include "stm32f722xx_init.h"
#include "stm32f722xx_usart.h"
...
void mriStm32f722xxUart_Init(Token *pParameterTokens)
{
...
static void saveUartToBeUsedByDebugger(uint32_t mriUart)
{
    ...
    mriStm32f722xxState.pCurrentUart = &g_uartConfigurations[mriUart-1];
}
...
{
    mriStm32f722xxState.pCurrentUart->pUartRegisters->CR1 |= USART_CR1_RXNEIE;
}
...
static int commUartIndex(void)
{
    return mriStm32f722xxState.pCurrentUart - g_uartConfigurations;
}

int Platform_CommHasReceiveData(void)
{
    return mriStm32f722xxState.pCurrentUart->pUartRegisters->ISR & USART_ISR_RXNE;
    // return Platform_FetchedByte || (mriStm32f722xxState.pCurrentUart->pUartRegisters->ISR & USART_ISR_RXNE);
}

int Platform_CommHasTransmitCompleted(void)
{
    return mriStm32f722xxState.pCurrentUart->pUartRegisters->ISR & USART_ISR_TC;
}
...
int Platform_CommReceiveChar(void)
{
    while(!Platform_CommHasReceiveData())
    {
        ...
        /* In case of overrun, blindly ignore */
        if (mriStm32f722xxState.pCurrentUart->pUartRegisters->ISR & USART_ISR_ORE)
        {
            mriStm32f722xxState.pCurrentUart->pUartRegisters->ICR |= USART_ICR_ORECF;
        }
    }
    return (mriStm32f722xxState.pCurrentUart->pUartRegisters->RDR & 0x1FF);
}
...
{
    USART_TypeDef *uart = mriStm32f722xxState.pCurrentUart->pUartRegisters;
    while (!(uart->ISR & USART_ISR_TXE))
    {
        /* busy wait */
    }
    uart->TDR = (Character & 0x1FF);
}
